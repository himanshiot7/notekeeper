...
if(isset($_POST['submit'])){
    ...
    if(mysqli_num_rows($result) > 0){
        ...
        // Verifying the entered password against the hashed password in the database
        if($password==$row["password"]){
            echo "Login successful!";
            header("location:note_keeper.php?idemail=".urlencode($email));
            exit();
        } else {
            echo "<script>alert('Invalid email or password.');</script>";
        }
    } else {
        echo "<script>alert('No account found with $email.');</script>";
    }
}
?>
